...
import torch
import warp as wp
import para_env  # Added para_env support
...
_ENV_NAME = flags.DEFINE_string(
    ...
    (
        ...
        f"{', '.join(mujoco_playground.registry.ALL_ENVS + para_env.ALL_ENVS)}"
    ),
)
...
def get_para_hand_rsl_rl_config(env_name: str) -> config_dict.ConfigDict:
    """Returns a default RSL-RL config for ParaHand environments."""
    config = config_dict.ConfigDict()
    
    # Policy Config
    config.policy = config_dict.ConfigDict()
    config.policy.class_name = "ActorCritic"
    config.policy.init_noise_std = 1.0
    config.policy.actor_hidden_dims = [512, 256, 128]
    config.policy.critic_hidden_dims = [512, 256, 128]
    config.policy.activation = "elu"

    # Algorithm Config
    config.algorithm = config_dict.ConfigDict()
    config.algorithm.class_name = "PPO"
    config.algorithm.value_loss_coef = 1.0
    config.algorithm.use_clipped_value_loss = True
    config.algorithm.clip_param = 0.2
    config.algorithm.entropy_coef = 0.01  
    config.algorithm.num_learning_epochs = 5
    config.algorithm.num_mini_batches = 4
    config.algorithm.learning_rate = 1e-3
    config.algorithm.schedule = "adaptive"  # RSL-RL specific: adaptive LR
    config.algorithm.gamma = 0.99
    config.algorithm.lam = 0.95
    config.algorithm.desired_kl = 0.01
    config.algorithm.max_grad_norm = 1.0

    # Runner Config
    # config.runner = config_dict.ConfigDict()
    # config.policy_class_name = "ActorCritic"   <-- moved to policy config
    # config.algorithm_class_name = "PPO"        <-- moved to algorithm config
    config.num_steps_per_env = 24  # Similar to unroll_length
    config.max_iterations = 1500  # Total training iterations
    config.save_interval = 50
    config.experiment_name = f"{env_name}_rsl_rl"
    config.run_name = ""
    config.resume = False
    config.load_run = "-1"
    config.checkpoint = -1
...
def get_rl_config(env_name: str) -> config_dict.ConfigDict:
  if env_name in para_env.ALL_ENVS:
      return get_para_hand_rsl_rl_config(env_name)
...
def main(argv):
  ...
  if _ENV_NAME.value in para_env.ALL_ENVS:
  ...
  # Logging directory
  if _PLAY_ONLY.value:
    logdir = os.path.join("logs", "playonly", exp_name)
  else:
  ...
  if _ENV_NAME.value in para_env.ALL_ENVS:
    raw_env = para_env.load(
        _ENV_NAME.value, config=env_cfg
    )
    # RSL-RL usually expects flattened observations, but let's see how the wrapper handles it.
    # The default Brax wrapper might need adjustment if using para_env which returns similar structs.
  else:
  ...
  camera.distance = 1.2           # 缩短摄像机距离
  camera.azimuth = -60           # 调整方位角以获得更好的视角
  camera.elevation = 0          # 从略微向下的角度观察
  camera.lookat[2] = 0.05          # 调整观察点高度
